/**
 * Soniox WebSocket Client — ported from desktop app
 * 
 * Proven patterns carried over:
 * - Auto-reconnect on transient errors (max 3 attempts)
 * - Seamless session reset every 3 min (make-before-break)
 * - Context carryover: recent translations sent as domain context
 * - Speaker diarization support
 * 
 * Reference: phuc-nt/my-translator/src/js/soniox.js
 */
...
// Session reset: 3 minutes (proven optimal from desktop)
...
export class SonioxClient {
  ...
  /**
   * Process Soniox response — token parsing (proven from desktop)
   */
  ...
  _handleApiError(data) {
    ...
    if (code === 401) userMessage = '❌ Invalid API key. Please check your key in Settings.';
    else if (code === 429) userMessage = '⏳ Rate limit exceeded. Please wait a moment.';
    else if (code === 402) userMessage = '💳 Insufficient credits. Check your Soniox account.';
    else if (code === 400) userMessage = `⚙️ Config error: ${message}`;
